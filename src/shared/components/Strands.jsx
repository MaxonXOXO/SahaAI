...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const update = t => {
      ...
      // Real-time audio reactive multipliers - very subtle
      const audioBoost = intensityRef?.current || 0;
      const dynamicAmplitude = current.amplitude + (audioBoost * 0.15);
      const dynamicThickness = current.thickness + (audioBoost * 0.02);
      const dynamicScale = current.scale + (audioBoost * 0.05);
      ...
      program.uniforms.uSpeed.value = current.speed; // Keep speed constant
      program.uniforms.uAmplitude.value = dynamicAmplitude;
      program.uniforms.uWaviness.value = current.waviness;
      program.uniforms.uThickness.value = dynamicThickness;
      ...
      program.uniforms.uScale.value = dynamicScale; // Scale up and down instead
